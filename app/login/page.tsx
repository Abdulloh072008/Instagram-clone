...
export default function LoginPage() {
  ...
  // Google-провайдер настроен на этом сервере? Если нет — кнопку не показываем,
  // чтобы не отправлять на ошибку «Missing client_id».
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // (2) локально знаем, что почта привязана к аккаунту <userName> — просим пароль
      ...
      // (3) спрашиваем сервер (работает на ЛЮБОМ устройстве — так у друга без
      // локальных данных вход не создаёт дубль, а просит пароль привязанного аккаунта)
      const serverLinked = await serverLinkedAccount(email);
      if (serverLinked) {
        setLinkedAcct(serverLinked);
        bridging.current = false;
        return;
      }

      // (4) почта нигде не привязана — детерминированный авто-аккаунт по почте
