...
- `Location/update-Location` → 400 `Missing type map configuration
  (UpdateLocationDto -> Location)` — сломан AutoMapper на сервере.
- `Post/delete-post` → 500, если у поста есть зависимые записи (коммент/лайк/
  избранное/сторис): сервер не умеет каскадно удалять. На «чистом» посте
  без зависимостей удаление работает.

Эти два — задача бэкенд-команды; клиентские вызовы корректны.
